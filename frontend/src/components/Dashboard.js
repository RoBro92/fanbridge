...
export function initDashboardContainer(container) {
  ...
  container.innerHTML = `
    ...
    <div id="tab-config-content" style="display: none;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 16px;">
            <div>
              ...
              <div style="display: flex; gap: 8px;">
                <input type="text" id="controller-name" class="input-base" value="" maxlength="${CONTROLLER_NAME_MAX}" autocomplete="off" style="flex: 1;">
                <button id="btn-rename-controller" class="btn btn-primary" style="padding: 4px 12px;">Save</button>
              </div>
              <span id="rename-controller-status" role="status" aria-live="polite" style="display: none; margin-top: 6px; font-size: 11px;"></span>
            </div>
      ...
      <div class="controller-tools-grid">
        <div id="serial-tools-container" class="controller-tool-slot"></div>
        <div id="updates-container" class="controller-tool-slot"></div>
      </div>

      <div id="logs-container"></div>

      <!-- Danger Zone stays last so destructive actions are isolated from routine controls. -->
      <div class="glass-card" style="margin: 24px 0; border: 1px solid hsla(0, 80%, 50%, 0.3);">
  ...
  const renameStatus = document.getElementById('rename-controller-status');

  inputName.addEventListener('input', () => {
    inputName.dataset.dirty = 'true';
    renameStatus.style.display = 'none';
  });

  btnRename.addEventListener('click', async () => {
    const controllerId = String(window.activeControllerId || inputName.dataset.controllerId || '');
    const newName = inputName.value.trim();
    if (!controllerId || !newName) {
      renameStatus.textContent = 'Enter a controller name.';
      renameStatus.style.color = 'var(--color-error)';
      renameStatus.style.display = 'block';
      return;
    }
    if (newName.length > CONTROLLER_NAME_MAX) {
      renameStatus.textContent = `Controller names are limited to ${CONTROLLER_NAME_MAX} characters.`;
      renameStatus.style.color = 'var(--color-error)';
      renameStatus.style.display = 'block';
      return;
    }
    ...
    btnRename.textContent = 'Saving…';
    renameStatus.style.display = 'none';
    try {
      await api.renameController(controllerId, newName);
      inputName.value = newName;
      inputName.dataset.controllerId = controllerId;
      inputName.dataset.dirty = 'false';

      document.querySelectorAll('.controller-nav-item').forEach((item) => {
        if (item.getAttribute('data-id') !== controllerId) return;
        item.setAttribute('data-name', newName);
        item.setAttribute('title', newName);
      ...
      btnRename.textContent = 'Saved';
    } catch (error) {
      renameStatus.textContent = error.message || 'Could not rename controller.';
      renameStatus.style.color = 'var(--color-error)';
      renameStatus.style.display = 'block';
      btnRename.textContent = 'Save';
    } finally {
      window.setTimeout(() => {
...
export function updateDashboardData(data, activeControllerId) {
  ...
  const controllerNameInput = document.getElementById('controller-name');
  if (controllerNameInput) {
    const controllerId = String(controller.id || '');
    const controllerName = String(controller.name || controllerId);
    const changedController = controllerNameInput.dataset.controllerId !== controllerId;
    const hasUnsavedEdit = controllerNameInput.dataset.dirty === 'true';
    if (changedController || (!hasUnsavedEdit && document.activeElement !== controllerNameInput)) {
      controllerNameInput.value = controllerName;
      controllerNameInput.dataset.controllerId = controllerId;
      controllerNameInput.dataset.dirty = 'false';
      const renameStatus = document.getElementById('rename-controller-status');
      if (renameStatus) renameStatus.style.display = 'none';
    }
  }
  ...
  updateDriveTable({
    ...data,
    drives: Array.isArray(controller.drives) ? controller.drives : [],
    hdd: controller.hdd,
    ssd: controller.ssd,
  ...
  const dashboardFans = document.getElementById('dashboard-fans-strip');
  ...
  if (dashboardFans) dashboardFans.style.display = isDIY ? 'none' : 'grid';
  if (hardwareTelemetry) hardwareTelemetry.style.display = isDIY ? 'none' : 'flex';
  if (fanConfigSection) fanConfigSection.style.display = isDIY ? 'none' : 'block';
  if (configOverview) configOverview.style.gridTemplateColumns = isDIY ? '1fr' : '1fr 1fr';
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (!fan) {
      badgeEl.innerHTML = '<span class="text-muted">Unknown</span>';
